...
RUN cd /StackWalk && ./setup.rb --no-packagemanager && rm -rf /StackWalk/ThirdParty && strip /StackWalk/build/ThirdParty/**/*.so*
...
COPY --from=builder /usr/local/etc/StackWalkAsAService/wt_config.xml /usr/local/etc/StackWalkAsAService/
COPY --from=builder /usr/local/var/StackWalkAsAService/http /usr/local/var/StackWalkAsAService/http

# WORKDIR /stackwalk/
ENTRYPOINT ["/usr/local/bin/stackwalkwebapp", "--docroot", "/usr/local/var/StackWalkAsAService/http", "-c", "/usr/local/etc/StackWalkAsAService/wt_config.xml", "--http-address", "0.0.0.0"]
